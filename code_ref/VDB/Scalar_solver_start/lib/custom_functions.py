...
def plot_prediction(model, input_data, output_data):
    ...
    def plot_examples(input_data, output_data, predicted_output):
        ...
        fig.colorbar(im2, ax=axes[0, 1], format=StrMethodFormatter('{x:01.3f}'))
        ...
        fig.colorbar(im3, ax=axes[1, 0], format=StrMethodFormatter('{x:01.3f}'))
        axes[1, 0].set_title('Difference')
        axes[1, 0].axis('off')

        im4 = axes[1, 1].imshow(predicted_output, cmap='jet', interpolation='none')
        im4.set_clim(vmin, vmax)
        fig.colorbar(im4, ax=axes[1, 1], format=StrMethodFormatter('{x:01.3f}'))
