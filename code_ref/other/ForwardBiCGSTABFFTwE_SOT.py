...
w_E_m, exit_code_m, information_m = ITERBiCGSTABwE(E_inc, CHI_eps, Errcri, x0=x0)
toc1 = time.time() - tic1
print("toc", toc1)
...
E_sct = KopE(w_E, gamma_0)
E_val = E(E_inc, E_sct)
plotEtotalwavefield(E_inc, a, X1, X2, N1, N2)

# Drop the first and last columns and rows due to finite differences at border
plotEtotalwavefield(E_sct[:, 1:-1, 1:-1], a, X1[1:-1, 1:-1], X2[1:-1, 1:-1], N1-1, N2-1)
plotEtotalwavefield(E_val[:, 1:-1, 1:-1], a, X1[1:-1, 1:-1], X2[1:-1, 1:-1], N1-1, N2-1)
