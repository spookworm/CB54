...
if exist(fullfile(cd, 'DATA2D.mat'), 'file')
    ...
    legend('Integral-equation method', 'Bessel-function method', 'Location', 'NorthEast');
    text(50, 0.8*max(abs(data)), ['Error^{sct} = ', error, '  '], 'EdgeColor', 'red', 'Fontsize', 11);
else
	plot(angle, abs(data), 'b')
